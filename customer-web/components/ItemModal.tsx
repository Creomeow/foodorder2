...
export default function ItemModal({
  ...
  // Pre-select defaults for each group, or the line's existing modifiers when editing.
  const [selected, setSelected] = useState<Record<string, string[]>>(() => {
    const init: Record<string, string[]> = {};
    for (const g of groups) {
      if (editingLine) {
        const optionIds = new Set(editingLine.modifiers.map((m) => m.optionId));
        init[g.id] = g.options.filter((o) => optionIds.has(o.id)).map((o) => o.id);
      } else {
  ...
  function save() {
    ...
    if (editingLine) {
      updateLine(editingLine.key, item, chosenOptions, notes, qty);
    } else {
      ...
      >
        ...
        <div className="flex items-center gap-3 border-t border-gray-100 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
          ...
          <Button className="flex-1" onClick={save}>
            {editingLine ? 'Save changes' : 'Add'} · {money(unit * qty)}
          </Button>
        </div>
      </div>
    </div>
